...
# --- 스킬 클래스 ---
class Skill:
    def __init__(self, name, level, max_level, rarity, use_count, effect, is_monster_only=False):
        self.name = name
        self.level = level
...
class Character:
    ...
    def take_damage(self, damage):
        ...
        ignore_defense_active = False
        for effect in self.status_effects:
            if effect.ignore_defense:
                ignore_defense_active = True
                break

        damage_taken_multiplier = 1.0
        for effect in self.status_effects:
            damage_taken_multiplier += effect.damage_taken_modifier

        calculated_defense = 0
        if not ignore_defense_active:
            calculated_defense = self.defense
...
class Game:
    ...
    def _initialize_skills(self):
        ...
        def siphon_life(caster, target, skill):
            ...
            target.take_damage(caster.attack * skill.level)
        ...
        def earthshatter(caster, target, skill):
            ...
            target.take_damage(caster.attack * 1.8)

        def exsanguinate(caster, target, skill):
            print(f"'{skill.name}' 발동... {target.name}의 피를 말린다.")
            target.add_status_effect(StatusEffect("과다출혈", 3, damage_per_turn=caster.attack * 0.1 * skill.level))
        ...
        def taunt(caster, target, skill):
            print(f"'{skill.name}' 발동... {target.name}을(를) 조롱하여 이성을 잃게 만든다.")
            pass
        ...
        def flurry(caster, target, skill):
            ...
            target.take_damage(caster.attack * 0.8)
            target.take_damage(caster.attack * 0.8)
        ...
        def devour(caster, target, skill):
            ...
            damage = caster.attack * 2.5
            target.take_damage(damage)
            caster.heal(damage * 0.5) # Devour heals the monster
        ...
        self.all_skills = [
            Skill("후벼파기", 1, 3, 1, 5, gouge),
            Skill("강철의 의지", 1, 3, 1, 3, iron_will),
            Skill("전쟁의 포효", 1, 3, 1, 3, war_cry),
            Skill("꿰뚫기", 1, 3, 1, 6, gouge),
            Skill("걷어차기", 1, 3, 1, 4, gouge),
            Skill("생명 흡수", 1, 3, 1, 2, siphon_life),
            Skill("조롱", 1, 3, 1, 2, taunt),
            Skill("약화", 1, 3, 1, 3, weaken),
            Skill("기절시키기", 1, 3, 1, 3, stun),
            Skill("증오 집중", 1, 3, 1, 2, focus_hate),
            Skill("부패의 손길", 1, 3, 1, 4, blight),
            Skill("사라지기", 1, 3, 1, 3, fade),
            Skill("정신 집중", 1, 2, 1, 2, focus_hate),
            Skill("뼈 갑옷", 1, 3, 1, 2, bone_armor),
            Skill("올가미", 1, 2, 1, 3, ensnare),
            Skill("죽음의 명상", 1, 3, 1, 1, death_trance),
            Skill("가시 갑옷", 1, 3, 1, 3, thorn_mail),
            Skill("암습", 1, 3, 1, 4, gouge),
            Skill("방해", 1, 3, 1, 5, weaken),
            Skill("생명 갈취", 1, 3, 1, 2, siphon_life),

            Skill("분쇄", 1, 3, 2, 4, pulverize),
            Skill("재생의 외침", 1, 2, 2, 3, siphon_life),
            Skill("피의 수확", 1, 2, 2, 3, reaping),
            Skill("과다출혈", 1, 3, 2, 3, exsanguinate),
            Skill("불구 만들기", 1, 2, 2, 2, cripple),
            Skill("뼈 부수기", 1, 3, 2, 3, shatter_bone),
            Skill("약점 노출", 1, 2, 2, 2, expose_weakness),
            Skill("칼날의 춤", 1, 3, 2, 3, flurry),
            Skill("저돌적 돌진", 1, 2, 2, 2, pulverize),
            Skill("피의 갈증", 1, 2, 2, 3, drain_life),
            Skill("침묵의 인장", 1, 2, 2, 2, silence),
            Skill("무모한 분노", 1, 2, 2, 2, reckless_abandon),
            Skill("거울 환영", 1, 2, 2, 2, mirror_image),
            Skill("사술", 1, 2, 2, 3, hex),
            Skill("정신 태우기", 1, 2, 2, 3, mind_sear),
            Skill("치명타", 1, 2, 2, 3, fatal_blow),
            Skill("이중 공격", 1, 3, 2, 4, flurry),
            Skill("흡혈", 1, 3, 2, 3, drain_life),
            Skill("광기", 1, 2, 2, 2, war_cry),
            Skill("재생력", 1, 3, 2, 1, siphon_life),

            Skill("처단", 1, 1, 3, 1, execute),
            Skill("방패 파괴", 1, 1, 3, 1, stun),
            Skill("그림자 형상", 1, 1, 3, 1, shadow_form),
            Skill("약자멸시", 1, 1, 3, 1, scorn_the_weak),
            Skill("파괴", 1, 1, 3, 1, pulverize),
            Skill("영혼 갈취", 1, 1, 3, 1, drain_life),
            Skill("복수", 1, 1, 3, 1, gouge),
            Skill("전장의 군주", 1, 1, 3, 1, war_cry),
            Skill("최후의 발악", 1, 1, 3, 1, iron_will),
            Skill("영웅적인 일격", 1, 1, 3, 1, execute),
            Skill("대지 분쇄", 1, 1, 3, 2, earthshatter),
            Skill("역병의 숨결", 1, 1, 3, 2, blight),
            Skill("그림자 암살", 1, 1, 3, 2, flurry),
            Skill("정화의 불길", 1, 1, 3, 1, lambda c, t, s: c.status_effects.clear()),
            Skill("어둠의 가호", 1, 1, 3, 1, bone_armor),
            Skill("파멸의 파동", 1, 1, 3, 2, gouge),
            Skill("피의 폭풍", 1, 1, 3, 2, exsanguinate),
            Skill("무력화의 저주", 1, 1, 3, 2, weaken),
            Skill("실명의 빛", 1, 1, 3, 2, stun),
            Skill("초월적인 존재", 1, 1, 3, 1, shadow_form),

            Skill("광전사의 격노", 1, 1, 4, 1, frenzy),
            Skill("대지 가르기", 1, 1, 4, 1, earthshatter),
            Skill("시간 왜곡", 1, 1, 4, 1, lambda c, t, s: print("시간의 흐름이 뒤틀린다.")),
            Skill("심판", 1, 1, 4, 1, execute),
            Skill("불멸", 1, 1, 4, 1, shadow_form),
            Skill("파멸의 일격", 1, 1, 4, 1, pulverize),
            Skill("영혼 포식", 1, 1, 4, 1, siphon_life),
            Skill("철옹성", 1, 1, 4, 1, iron_will),
            Skill("고대의 외침", 1, 1, 4, 1, war_cry),
            Skill("세계의 종말", 1, 1, 4, 1, earthshatter),
            Skill("신벌", 1, 1, 4, 1, execute),
            Skill("유성우", 1, 1, 4, 1, earthshatter),
            Skill("차원 붕괴", 1, 1, 4, 1, pulverize),
            Skill("세계수", 1, 1, 4, 1, siphon_life),
            Skill("아마겟돈", 1, 1, 4, 1, earthshatter),
            Skill("절대자의 권능", 1, 1, 4, 1, frenzy),
            Skill("영겁의 나락", 1, 1, 4, 1, stun),
            Skill("태초의 광기", 1, 1, 4, 1, reckless_abandon),
            Skill("존재 소멸", 1, 1, 4, 1, execute),
            Skill("공허 창조", 1, 1, 4, 1, siphon_life),

            Skill("전염병", 1, 1, 5, 99, pestilence, is_monster_only=True),
            Skill("석화의 시선", 1, 1, 5, 99, petrifying_gaze, is_monster_only=True),
            Skill("영혼 흡수 오라", 1, 1, 5, 99, soul_drain_aura, is_monster_only=True),
            Skill("혐오체 소환", 1, 1, 5, 99, summon_abomination, is_monster_only=True),
            Skill("죽음의 손아귀", 1, 1, 5, 99, execute, is_monster_only=True),
            Skill("공포의 눈빛", 1, 1, 5, 99, weaken, is_monster_only=True),
            Skill("부패시키는 저주", 1, 1, 5, 99, exsanguinate, is_monster_only=True),
            Skill("지옥불 폭풍", 1, 1, 5, 99, earthshatter, is_monster_only=True),
            Skill("영혼의 절규", 1, 1, 5, 99, silence, is_monster_only=True),
            Skill("포식", 1, 1, 5, 99, devour, is_monster_only=True),
            Skill("화염 숨결", 1, 1, 5, 99, fire_breath, is_monster_only=True),
            Skill("빙결 숨결", 1, 1, 5, 99, frost_breath, is_monster_only=True),
            Skill("독액 분출", 1, 1, 5, 99, poison_breath, is_monster_only=True),
            Skill("소용돌이", 1, 1, 5, 99, whirlpool, is_monster_only=True),
        ]
    ...
    def _initialize_monsters(self):
        ...
        self.all_monsters.extend([
            Monster("광포한 오크", 2, False, 40, 10, 5, 0.1, 0.1, 10, [self.get_skill("분쇄")]),
            Monster("전쟁광 홉고블린", 2, False, 35, 12, 6, 0.1, 0.1, 12, [self.get_skill("전쟁의 포효")]),
            Monster("썩어가는 놀", 2, False, 30, 11, 4, 0.2, 0.1, 11, [self.get_skill("부패의 손길")]),
            Monster("굶주린 늑대", 2, False, 28, 13, 3, 0.3, 0.1, 10, [self.get_skill("과다출혈")]),
            Monster("해골 병사", 2, False, 33, 10, 8, 0.1, 0.1, 13, [self.get_skill("뼈 부수기")]),
            Monster("오크 전쟁군주", 2, True, 120, 15, 8, 0.1, 0.2, 100, [self.get_skill("분쇄"), self.get_skill("전쟁의 포효")]),
        ])
        self.all_monsters.extend([
            Monster("걸어다니는 시체", 3, False, 50, 15, 8, 0.1, 0.1, 15, [self.get_skill("생명 갈취")]),
            Monster("시체 포식자 구울", 3, False, 45, 17, 6, 0.1, 0.1, 17, [self.get_skill("포식")]),
            Monster("울부짖는 와이트", 3, False, 40, 16, 5, 0.2, 0.1, 16, [self.get_skill("영혼의 절규")]),
            Monster("돌가죽 가고일", 3, False, 55, 14, 10, 0.1, 0.1, 18, [self.get_skill("석화의 시선")]),
            Monster("탐욕의 미믹", 3, False, 60, 18, 12, 0.05, 0.2, 25, [self.get_skill("기절시키기")]),
            Monster("고대 리치", 3, True, 200, 25, 15, 0.1, 0.2, 150, [self.get_skill("죽음의 손아귀"), self.get_skill("뼈 갑옷"), self.get_skill("침묵의 인장")]),
        ])
        self.all_monsters.extend([
            Monster("미궁의 미노타우르스", 4, False, 80, 22, 10, 0.1, 0.15, 22, [self.get_skill("저돌적 돌진")]),
            Monster("비명지르는 하피", 4, False, 60, 20, 8, 0.3, 0.1, 20, [self.get_skill("영혼의 절규")]),
            Monster("복수심에 불타는 켄타우로스", 4, False, 70, 25, 9, 0.2, 0.1, 24, [self.get_skill("칼날의 춤")]),
            Monster("폭풍의 그리폰", 4, False, 75, 24, 12, 0.15, 0.1, 26, [self.get_skill("실명의 빛")]),
            Monster("외눈의 사이클롭스", 4, False, 100, 28, 15, 0.05, 0.1, 30, [self.get_skill("대지 분쇄")]),
            Monster("석화시키는 메두사", 4, True, 280, 35, 18, 0.2, 0.25, 250, [self.get_skill("석화의 시선"), self.get_skill("독액 분출")]),
        ])
        self.all_monsters.extend([
            Monster("흑요석 골렘", 5, False, 120, 30, 20, 0.0, 0.1, 35, [self.get_skill("강철의 의지")]),
            Monster("독액의 와이번", 5, False, 100, 35, 15, 0.2, 0.15, 38, [self.get_skill("역병의 숨결")]),
            Monster("죽음의 시선 바실리스크", 5, False, 110, 32, 18, 0.1, 0.1, 36, [self.get_skill("석화의 시선")]),
            Monster("심연의 나가", 5, False, 90, 38, 16, 0.25, 0.15, 40, [self.get_skill("침묵의 인장")]),
            Monster("타락한 서큐버스", 5, False, 80, 40, 14, 0.3, 0.2, 45, [self.get_skill("생명 갈취")]),
            Monster("악몽의 키메라", 5, True, 400, 45, 25, 0.15, 0.3, 400, [self.get_skill("화염 숨결"), self.get_skill("독액 분출"), self.get_skill("빙결 숨결")]),
        ])
        self.all_monsters.extend([
            Monster("심해의 정령", 6, False, 130, 40, 25, 0.2, 0.1, 45, [self.get_skill("올가미")]),
            Monster("화염의 정령", 6, False, 130, 45, 22, 0.2, 0.15, 48, [self.get_skill("지옥불 폭풍")]),
            Monster("소용돌이의 정령", 6, False, 120, 42, 20, 0.3, 0.1, 46, [self.get_skill("칼날의 춤")]),
            Monster("대지의 정령", 6, False, 150, 38, 30, 0.1, 0.1, 50, [self.get_skill("대지 분쇄")]),
            Monster("그림자 암살자", 6, False, 110, 50, 18, 0.4, 0.25, 55, [self.get_skill("그림자 암살")]),
            Monster("심연의 히드라", 6, True, 550, 55, 30, 0.1, 0.2, 550, [self.get_skill("재생력"), self.get_skill("독액 분출"), self.get_skill("저주")]),
        ])
        self.all_monsters.extend([
            Monster("지옥의 병사", 7, False, 160, 55, 30, 0.1, 0.15, 60, [self.get_skill("전쟁의 포효")]),
            Monster("비열한 임프", 7, False, 140, 52, 28, 0.25, 0.1, 58, [self.get_skill("사라지기")]),
            Monster("지옥견", 7, False, 150, 60, 25, 0.3, 0.15, 65, [self.get_skill("과다출혈")]),
            Monster("뼈의 용", 7, False, 200, 65, 40, 0.1, 0.2, 80, [self.get_skill("뼈 갑옷")]),
            Monster("타락한 천사", 7, False, 180, 70, 35, 0.2, 0.25, 90, [self.get_skill("신벌")]),
            Monster("거대한 베히모스", 7, True, 750, 70, 40, 0.05, 0.25, 700, [self.get_skill("대지 분쇄"), self.get_skill("철옹성")]),
        ])
        self.all_monsters.extend([
            Monster("서리 거인", 8, False, 220, 70, 40, 0.1, 0.15, 80, [self.get_skill("빙결 숨결")]),
            Monster("용암 거인", 8, False, 220, 75, 38, 0.1, 0.15, 85, [self.get_skill("화염 숨결")]),
            Monster("폭풍 거인", 8, False, 200, 72, 35, 0.2, 0.15, 82, [self.get_skill("실명의 빛")]),
            Monster("고대 용거북", 8, False, 300, 65, 60, 0.0, 0.1, 100, [self.get_skill("철옹성")]),
            Monster("죽음의 기사", 8, False, 250, 80, 50, 0.15, 0.2, 110, [self.get_skill("죽음의 손아귀")]),
            Monster("심해의 레비아탄", 8, True, 1000, 85, 50, 0.1, 0.3, 900, [self.get_skill("대지 가르기"), self.get_skill("소용돌이")]),
        ])
        self.all_monsters.extend([
            Monster("고위 악마", 9, False, 280, 85, 50, 0.15, 0.2, 120, [self.get_skill("지옥불 폭풍")]),
            Monster("심연의 감시자", 9, False, 260, 82, 48, 0.2, 0.2, 115, [self.get_skill("약점 노출")]),
            Monster("공허의 약탈자", 9, False, 250, 90, 45, 0.25, 0.2, 125, [self.get_skill("차원 붕괴")]),
            Monster("고대 정령", 9, False, 350, 80, 60, 0.1, 0.15, 140, [self.get_skill("정화의 불길")]),
            Monster("신화 속 야수", 9, False, 400, 95, 55, 0.1, 0.25, 150, [self.get_skill("태초의 광기")]),
            Monster("타락한 대천사", 9, True, 1300, 100, 60, 0.2, 0.35, 1200, [self.get_skill("신벌"), self.get_skill("심판"), self.get_skill("영혼 포식")]),
        ])
        self.all_monsters.extend([
            Monster("고룡", 10, False, 500, 100, 70, 0.1, 0.25, 200, [self.get_skill("화염 숨결")]),
            Monster("서리 고룡", 10, False, 480, 110, 65, 0.15, 0.25, 220, [self.get_skill("빙결 숨결")]),
            Monster("암흑 고룡", 10, False, 450, 120, 60, 0.2, 0.25, 250, [self.get_skill("역병의 숨결")]),
            Monster("분노의 고룡", 10, False, 550, 130, 80, 0.1, 0.3, 300, [self.get_skill("광전사의 격노")]),
            Monster("황금 고룡", 10, False, 600, 110, 90, 0.1, 0.2, 280, [self.get_skill("신의 가호")]),
    ...
    def get_skill(self, name):
        for skill in self.all_skills:
            if skill.name == name:
                return Skill(skill.name, skill.level, skill.max_level, skill.rarity, skill.initial_use_count, skill.effect, skill.is_monster_only)
        return None
    ...
    def start(self):
        ...
        if self.player.is_alive():
            print("당신의 발자취는 피로 쓰였고, 이곳엔 아무것도 남아있지 않다")
            time.sleep(2)
            print("끈질긴 생명이다. 하지만 이 저주받은 땅에서 네놈의 여정은 끝나지 않았다.\n")
            time.sleep(2)
    ...
    def player_turn(self, monster):
        ...
        for i, skill in enumerate(self.player.skills):
            print(f"{i+2}. {skill.name} (남은 사용: {skill.use_count})\n")

        while True:
            try:
                choice = int(input("행동을 선택하세요: "))
    ...
    def skill_acquisition(self, is_boss):
        print("\n어둠 속에서 새로운 힘이 느껴진다...\n")
        time.sleep(1)
        available_skills = [s for s in self.all_skills if s.level < s.max_level and not s.is_monster_only]
        
        choices = [] 

        if is_boss:
            hero_or_legend_skills = [s for s in available_skills if s.rarity >= 3]
            other_skills = [s for s in available_skills if s.rarity < 3]

            if hero_or_legend_skills:
                choices.extend(random.sample(hero_or_legend_skills, min(1, len(hero_or_legend_skills))))
                other_skills = [s for s in other_skills if s not in choices]

            remaining_slots = 3 - len(choices)
            if remaining_slots > 0 and other_skills:
                choices.extend(random.sample(other_skills, min(remaining_slots, len(other_skills))))
            
            if len(choices) < 3:
                all_remaining_skills = [s for s in available_skills if s not in choices]
                choices.extend(random.sample(all_remaining_skills, min(3 - len(choices), len(all_remaining_skills))))

        else:
            if available_skills:
                weights = [10 / s.rarity for s in available_skills]
                choices = random.choices(available_skills, weights=weights, k=min(3, len(available_skills)))
            else:
                choices = []
        ...
        print("어떤 힘을 받아들이겠는가, 같은 힘을 고르면 해당 힘이 더욱 강해진다:\n")
        time.sleep(1)
        for i, skill in enumerate(choices):
            print(f"{i+1}. {skill.name} (시전 가능 횟수: {skill.use_count}, 희귀도: {skill.rarity}, 레벨: {skill.level if self.player_has_skill(skill.name) else 0})\n")
    ...
    def add_or_level_up_skill(self, skill_to_add):
        for skill in self.player.skills:
            if skill.name == skill_to_add.name:
                ...
                print(f"금지된 기술이 더욱 강해졌다! '{skill.name}'이(가) Lv.{skill.level}이 되었다.\n")
                return
    ...
    def shop(self):
        ...
        while True:
            ...
            for i, item in enumerate(shop_inventory):
                ...
                print(f"    (생명: {item.health}, 공격: {item.attack}, 방어: {item.defense})\n")
            ...
            if choice == len(shop_inventory) + 1:
                ...
                if self.player.gold >= chosen_item.price:
                    ...
                    shop_inventory.pop(choice-1) # Remove item from shop after purchase
                else:
                    print("금화가 부족하다.")
            time.sleep(2)
